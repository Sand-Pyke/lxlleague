...
const user = (request: NextRequest) => ({
  login: Boolean(loggedIn(request)),
  username: loggedIn(request) || "",
  is_admin: loggedIn(request) === "admin",
});
...
export async function GET(
  ...
  if (path === "match/list")
    return NextResponse.json({
      match_list: matches.map((m) => ({
        ...m,
        player_count: m.playerCount,
        team_count: m.teamCount,
      })),
    });
  if (path === "players")
    return NextResponse.json({
...
export async function POST(
  ...
  if (path === "login" || path === "register") {
    ...
    response.cookies.set("lspl_user", username, {
      httpOnly: true,
      sameSite: "lax",
      path: "/",
    });
  ...
  if (path.startsWith("match/cancel/"))
    return NextResponse.json({ ok: true, message: "已取消报名" });
