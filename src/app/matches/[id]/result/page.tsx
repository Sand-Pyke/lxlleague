  ...
  return (
    <LeagueShell>
      ...
      <section className="result-hero">
        ...
        <span>{match.date} · 比赛已结束</span>
      </section>
      <section className="panel game-tabs">
        <button className="selected">GAME 1</button>
        ...
        {players.map((p, i) => (
          <div className="result-row" key={p.id}>
            <div>
              ...
              <b>{p.name}</b>
              <small>{i < 5 ? match.teams[0] : match.teams[1]}</small>
            </div>
            <span>
              {8 + (i % 4)} / {2 + (i % 3)} / {6 + (i % 5)}
            </span>
            <span>{p.kda}</span>
            <span>{(12 + i * 1.7).toFixed(1)}k</span>
            <span>{(9 + i * 0.6).toFixed(1)}k</span>
          </div>
        ))}
      </section>
    </LeagueShell>
